        ...
        initCubeportfolio: function () {
            ...
            this.collection.each(function (i, el) {
                ...
                var $this = $(el),
                    ...
                    setMeidaQueries = JSON.parse(el.getAttribute('data-components-queries'));
                $this.cubeportfolio({
                    filters: setControls,
    ...
    $.GSCore.components.GSFileAttachment = {
        ...
        _baseConfig: {
            changeInput: '<div class="u-file-attach-v3 g-mb-15">\
               				<div class="g-font-size-16 g-color-gray-dark-v2 mb-0">\
               				    Перетащите сюда нужные изображения \
               				    <span class="g-color-primary">Либо загрузите через Проводник</span>\
               				</div>\
               				<p class="g-font-size-14 g-color-gray-light-v2 mb-0">Не более 10 Мегабайт</p>\
              			</div>',
            showThumbs: true,
            templates: {
                box: '<div class="js-result-list row"></div>',
                item: '<div class="d-none preload-item js-result-list__item col-md-3 text-center">\
            ...
            uploadFile: {
                url: '/ajax.php',
                data: {albumId: $('input[name="fileAttachment2[]"]').attr('data-albumid')},
                type: 'POST',
                enctype: 'multipart/form-data',
                beforeSend: function () {
                    $('.u-file-attach-v3')[0].classList.add('_preloader');
                },
                success: function (response, element) {

                    $('.u-file-attach-v3')[0].classList.remove('_preloader');
                    response = JSON.parse(response);

                    let parent = element.find('.u-progress-bar-v1').parent();
